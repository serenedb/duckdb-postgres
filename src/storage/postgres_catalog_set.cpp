...
void PostgresCatalogSet::DropEntry(PostgresTransaction &transaction, DropInfo &info) {
	...
	lock_guard<mutex> entry_guard(entry_lock);
	if (entries.erase(info.Name().GetIdentifierName()) == 0) {
		return;
	}
	auto name_it = entry_map.find(info.Name().GetIdentifierName());
	if (name_it != entry_map.end() && name_it->second == info.Name().GetIdentifierName()) {
